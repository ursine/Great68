...
//	emu_timer *m_timer;                             // General timer
	...
	uint8_t m_cur_fa, m_cur_fc, m_cur_va;
	uint8_t m_cur_f1, m_cur_f2, m_cur_f2q, m_cur_f3;

	// Current committed values
	uint8_t m_filt_fa, m_filt_fc, m_filt_va;             // Analog parameters, noise volume, noise freq cutoff and voice volume, 4 bits each
	uint8_t m_filt_f1, m_filt_f2, m_filt_f2q, m_filt_f3; // Analog parameters, formant frequencies/Q on 4 bits except f2 on 5 bits

	// Internal state
	bool m_cur_closure;                             // Current internal closure state
	uint16_t m_noise;                                    // 15-bit noise shift register
	...
	uint8_t  m_pitch;                               // 7-bits pitch counter
	uint8_t  m_closure;                             // 5-bits glottal closure counter
	uint8_t  m_update_counter;                      // 6-bits counter for the 625Hz (main/1152) and 208Hz (main/3456) update timing generators
	...
	void build_standard_filter(double* a, double* b,
		...
		double c4) const; // Cap over second op-amp
	...
	void build_noise_shaper_filter(double *a, double *b,
		...
		double c4) const; // Switched cap after second amp-op

	void build_lowpass_filter(double *a, double *b,
		double c1t,  // Unswitched cap, over amp-op, top
		double c1b) const; // Switched cap, over amp-op, bottom
};
