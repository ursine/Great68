...
//  inflection bits
//-------------------------------------------------
void votrax_sc01_device::inflection_w(const uint8_t d)
{
	// only 2 bits matter
	const auto data = d & 3;
	if(m_inflection == data) return;
...
void votrax_sc01_device::phone_commit() {
	...
	for (int i=0; i<64; i++) {
	    ...
	    if (m_phone == ((val >> 56) & 0x3f)) {
			...
			std::printf("commit fa=%x va=%x fc=%x f1=%x f2=%x f2q=%x f3=%x dur=%02x cld=%x vd=%d cl=%d pause=%d\n",
...
void votrax_sc01_device::filters_commit(bool force)
{
	...
	if (m_filt_fa | m_filt_va | m_filt_fc | m_filt_f1 | m_filt_f2 | m_filt_f2q | m_filt_f3) {
		std::printf("filter fa=%x va=%x fc=%x f1=%x f2=%02x f2q=%x f3=%x\n",
...
*/
void votrax_sc01_device::build_standard_filter(double* const a, double* const b,
	...
	const double c4) const  // Cap over second op-amp
{
	...
	// is not needed.
	const double k0 = c1t / (m_cclock * c1b);
	const double k1 = c4 * c2t / (m_cclock * c1b * c3);
	const double k2 = c4 * c2b / (m_cclock * m_cclock * c1b * c3);

	// Estimate the filter cutoff frequency
	const double fpeak = sqrt(fabs(k0*k1 - k2))/(2*M_PI*k2);

	// Turn that into a warp multiplier
	const double zc = 2*M_PI*fpeak/tan(M_PI*fpeak / m_sclock);
...
void votrax_sc01_device::build_lowpass_filter(double* const a, double* const b,
	const double c1t, // Unswitched cap, over amp-op, top
	const double c1b) const // Switched cap, over amp-op, bottom
{
	...
	const double k = c1b / (m_cclock * c1t) * (150.0/4000.0);
...
/*
  ...
  We assume r0 = r2
*/
void votrax_sc01_device::build_noise_shaper_filter(
	...
	const double c4) const // Switched cap after second amp-op
{
	// Coefficients of H(s) = k1*s / (1 + k2*s + k3*s^2)
	const double k0 = c2t*c3*c2b/c4;
	const double k1 = c2t*(m_cclock * c2b);
	const double k2 = c1*c2t*c3/(m_cclock * c4);
...
/*
  ...
  that H(infinity)=1.
*/
void votrax_sc01_device::build_injection_filter(double* const a, double* const b,
			const double c1b, // Switched cap, input, bottom
			const double c2t, // Unswitched cap, over first amp-op, top
			const double c2b, // Switched cap, over first amp-op, bottom
			const double c3,  // Cap between the two op-amps
			const double c4) const  // Cap over second op-amp
{
	...
	const double k0 = m_cclock * c2t;
	const double k1 = m_cclock * (c1b * c3 / c2t - c2t);
	const double k2 = c2b;

	// Don't pre-warp
	const double zc = 2*m_sclock;
